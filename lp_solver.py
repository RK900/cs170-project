...
def solve(graph, list_locations, list_houses, starting_car_location):
    ...
    x = [m.add_var(name='car_taken_{}'.format(u, v), var_type=BINARY) for (u, v) in G.edges()]
    ...
    ta_travel = 1 * xsum(xsum(T[loc][house] * shortest_path_all_pairs[loc][house] * w[loc]
                              for loc in list_locations) for ta in list_of_homes)
